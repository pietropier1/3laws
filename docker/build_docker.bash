...
REGEX_QUERY="${PACKAGE_NAME}_[0-9]\+\.[0-9]\+\.[0-9]\+-[0-9]\+_amd64"
...
docker build --rm \
  --build-arg "LLL_USER_ID=$(id -u)" \
  --build-arg "LLL_GROUP_ID=$(id -g)" \
  --build-arg "LLL_DIALOUT_GID=$(getent group dialout | cut -d: -f3)" \
  --build-arg "LLL_INPUT_GID=$(getent group input | cut -d: -f3)" \
